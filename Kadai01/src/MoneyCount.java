...
public class MoneyCount {
	...
	public static final int[]  	SORTED_YENS = Arrays.stream(YENS).boxed().sorted(Comparator.reverseOrder()).mapToInt(Integer::intValue).toArray(); // YENSを降順にソートする
	...
	static int enterMoney() {
		...
		try(BufferedReader in = new BufferedReader(new InputStreamReader(System.in));) {
			while(true) {											// 正しい値が入力されるまで繰り返す
				...
				try {
					int numInput = Integer.parseInt(input);	// 標準入力を受け取る
					if(!validation(numInput))							// 正しい値か検証
						continue;
					return numInput;									// inputを返却
